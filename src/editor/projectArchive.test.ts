...
describe(".perler project archives", () => {
  it("round-trips typed cell data and metadata", async () => {
    ...
    const bytes = await exportPerlerProject(document);
    const restored = await importPerlerProject(bytes);
  ...
  it("rejects non-archive input", async () => {
    await expect(importPerlerProject(strToU8("not a zip"))).rejects.toThrow();
  });
  ...
  it("rejects archive path traversal", async () => {
    ...
    await expect(importPerlerProject(archive)).rejects.toThrow("不允许的文件");
  });
});
